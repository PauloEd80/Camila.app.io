...
// ════════════════════════════════════════
/**
 * Carrega dados do localStorage e normaliza IDs para string.
 * Versões antigas gravavam id como Number (Date.now()).
 * O código atual usa strings — a normalização garante compatibilidade.
 */
function carregarDados(key) {
  try {
    const dados = JSON.parse(localStorage.getItem(key));
    if (!Array.isArray(dados)) return null;
    // Converte qualquer id numérico legado para string
    // Normaliza id e pacienteId (agenda) para string
    return dados.map(item => ({
      ...item,
      id: String(item.id),
      ...(item.pacienteId !== undefined && { pacienteId: String(item.pacienteId) })
    }));
  } catch { return null; }
}
